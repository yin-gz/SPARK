...
def parse_args():
    ...
    parser.add_argument('--LABEL_TYPE', type=str, default="id", choices= ["id", "text", "both"], help='Label type for entity')
    ...
    parser.add_argument('--LOAD_LLM', default=False, action="store_true", help='Load LLM Result')
    ...
    parser.add_argument('--DATASET', type=str, default="icews14", help='Name of dataset')
    ...
    parser.add_argument('--LOAD_RULE', type=bool, default=False, help='Load rules for TLogic')
    
    #! Training parameters
    parser.add_argument("--TRAIN_BS", type=int, default=128, choices = [128, 256, 512], help="Batch size for eval")
    parser.add_argument("--EVAL_BS", type=int, default=128, choices = [128, 256, 512], help="Batch size for train")
    ...
    parser.add_argument('--LOSS_TYPE', type=str, default = "target_loss", choices= ["target_loss" , "all_loss"], help = 'Calculate loss only for predicted target or for all entities')
...
if __name__ == "__main__":
    ...
    if args.RUN_NAME is None:
            model_name = f"{args.DATASET}-{args.MODEL_NAME.split('/')[-1].split('-')[0]}-{args.ADAPTER_NAME}, bs:{args.TRAIN_BS},dim:{args.EMBEDDING_DIM}, loadLLM:{args.LOAD_LLM}, TEST_ADAPTER: {args.TEST_ADAPTER}, {args.TRAIN_PROP}"
            args.RUN_NAME = generate_run_name(model_name)
            print("gen_name", args.RUN_NAME)
    print(args)
                
    if args.WANDB:
        os.environ["WANDB_MODE"] = "offline"
        wandb.init(project = "TKG-" + args.DATASET, config=args)
    ...
    tlr_data = prepare_data(args) # load instruct data preprocessed by GENTKG
